...
export function HeroForm({ business, area, keyword }: HeroFormProps) {
  ...
  return (
    <div className="bg-white rounded-xl md:rounded-2xl p-3 md:p-6 shadow-2xl">
      <div className="mb-2 md:mb-3 pb-2 md:pb-3 border-b border-gray-200">
        <div className="flex items-center gap-2 text-amber-600 mb-0.5 md:mb-1">
          <Sparkles className="w-3.5 h-3.5 md:w-4 md:h-4" />
          <span className="text-[10px] md:text-xs font-medium uppercase tracking-wider">Quick Enquiry</span>
        </div>
        <h3 className="text-base md:text-xl font-bold text-gray-900">
          {config.title}
        </h3>
        <p className="text-[10px] md:text-xs text-gray-600">{config.subtitle}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-2 md:space-y-3">
              ...
              <div className="flex flex-wrap gap-1 md:gap-1.5">
                {field.options.map((option) => (
                  <button
                    ...
                    className={`px-2 md:px-2.5 py-0.5 md:py-1 text-[10px] md:text-xs rounded-full border transition-all ${
        ...
        <button
          ...
          className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2.5 md:py-3 rounded-lg flex items-center justify-center gap-2 transition-all disabled:opacity-70 text-sm md:text-base"
        >
